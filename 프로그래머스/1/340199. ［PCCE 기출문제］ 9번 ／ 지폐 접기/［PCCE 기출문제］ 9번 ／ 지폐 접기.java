...
class Solution {
    ...
    static private boolean isUnder(int[] wallet, int[]bill){
        for (int i = 0; i < wallet.length; i ++){
            if (wallet[i] < bill[i]) return false;
        }
        return true;
    }
    
    static private void turn90(int[]bill){
        int temp = bill[1];
        bill[1] = bill[0];
        bill[0] = temp;
    }
    
    static private void half(int[] bill){
        Arrays.sort(bill);
        bill[1] = bill[1]/2;
    }
}
